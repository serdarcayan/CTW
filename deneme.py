...
class Node(QGraphicsItem):
    ...
    def calculate_forces(self):
        ...
        self._new_pos.setX(
            min(max(self._new_pos.x(), scene_rect.left() + 10), scene_rect.right() - 10)
        )
        self._new_pos.setY(
            min(max(self._new_pos.y(), scene_rect.top() + 10), scene_rect.bottom() - 10)
        )
...
class GraphWidget(QGraphicsView):
    ...
    def draw_background(self, painter, rect):
        ...
        right_shadow = QRectF(
            scene_rect.right(), scene_rect.top() + 5, 5, scene_rect.height()
        )
        bottom_shadow = QRectF(
            scene_rect.left() + 5, scene_rect.bottom(), scene_rect.width(), 5
        )
